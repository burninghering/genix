...
router.post('/regist_user', async function (req, res) {
    ...
    if(await Instance.accountCheck(decryptID,decryptPW) && await Instance.nickNameCheck(userNickname)){
...
router.post('/modify_myinfo', async function (req, res) {
    ...
    if(await Instance.accountCheck(userID,decryptCurPW) && await Instance.nickNameCheck(userNickname)){
        db_manager.ModifyMyInfo(userID, decryptCurPW, decryptNewPW, userNickname, res);
    }
    else{
        res.status(400).send('잘못된 정보를 입력하셨습니다.');
    }
