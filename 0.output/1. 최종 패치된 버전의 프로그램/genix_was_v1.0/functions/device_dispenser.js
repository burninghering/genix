    ...
    if (!Instance.runState) {
        for (let i = 0; i < Instance.arrayList.length; i++) {
            ...
            switch (Instance.arrayList[i].DEV_TYPE_ID) {
                case 1: // MQTT
                    mqtt_connector.MQTT_RUN(DEV_ENDPOINT_URL, Instance.arrayList[i].DEV_NAME);
                    break;
                case 2: // UART
                    break;
                case 3: // OPC-UA
                    opcua.startOPCUAClient(DEV_ENDPOINT_URL, Instance.arrayList[i].DEV_ENDPOINT.split('@')[1]);
                    break;
                case 4: // CCTV
                    break;
            }
