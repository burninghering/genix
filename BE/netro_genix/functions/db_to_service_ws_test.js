      ...
      try {
        ...
        if (vesselData.length === 0) {
          ...
          vesselData.forEach((data) => {
            if (data.id === 50 || data.id === 51) {
              console.log(
                `Before fixing, devId ${data.id} has rcv_datetime:`,
                data.rcv_datetime
              )

              // log_datetime이 존재하는지 확인하고 올바르게 변환
              if (data.log_datetime && typeof data.log_datetime === "string") {
                // log_datetime을 rcv_datetime에 대입 (밀리초 제거)
                let fixedDatetime = new Date(data.log_datetime)
                  .toISOString()
                  .slice(0, 19)
                  .replace("T", " ")
                data.rcv_datetime = fixedDatetime
              } else {
                console.error(
                  `Invalid log_datetime for devId ${data.id}:`,
                  data.log_datetime
                )
              }
