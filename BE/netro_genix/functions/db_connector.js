...
async function SaveDummyData(
  ...
  try {
    ...
    await connection.execute(sql, [ devId || null, senId || null, senName || null, idVluType, dftValue, maxValue, minValue, unit, ])
  ...
  if (paramAirDataList.length == 270) {
    ...
    try {
      ...
      for (const chunk of chunks) {
        ...
        const values = chunk.flatMap((item) => [ item.logDateTime, item.devId, item.senId, item.sensorValue, ])
        console.log(values)
...
async function SaveOceanSysSensor(
  ...
  try {
    ...
    await connection.execute(sql, [ devId, senId, senName, idVluType, dftValue, maxValue, minValue, unit, ])
