...
// 선박 데이터를 가져오는 함수 (최신 데이터만 가져오기)
async function fetchVesselData() {
  const connection = await getConnection()
  try {
    // 각 선박에 대해 가장 최신의 데이터만 가져오는 쿼리
    const [vesselData] = await connection.query(`
            ...
            INNER JOIN (
                ...
                WHERE dev_id BETWEEN 1 AND 100
                GROUP BY dev_id
            ) latest ON v.dev_id = latest.dev_id AND v.log_datetime = latest.latest_log_datetime
      ...
      vesselData.forEach((item) => {
        if (item.dev_id === devId) {
          ...
          if (
            ...
            result.lati = parseFloat(item.sen_value) // 'lati' 값 매핑
          } else {
            result[item.sen_name.toLowerCase()] = parseFloat(item.sen_value) // 다른 센서 값 할당
          }
        }
      })
