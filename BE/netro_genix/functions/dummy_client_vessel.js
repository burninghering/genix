  ...
  ws.on("message", async (message) => {
    try {
      ...
      if (validateVesselData(data)) {
        ...
        // 202개의 데이터가 모두 수신된 경우에만 타임스탬프 확인
        if (vesselDataCache.size === 202) {
          ...
          if (uniqueTimestamps.size === 1) {
            ...
            console.log(`✅ 202개의 데이터가 동일한 타임스탬프(${lastValidTimestamp})로 수신되었습니다.`);
          } else {
            // 202개 데이터가 도달한 후에만 경고 메시지 출력
  ...
  const interval = setInterval(() => {
    ...
    if (uniqueTimestamps.size === 1) {
      ...
      if (currentTimestamp === lastValidTimestamp) {
        ...
        console.log(
          `✅ 202개의 데이터를 나누어 클라이언트로 전송했습니다. 타임스탬프: ${currentTimestamp}`
        );
      } else {
...
console.log("Broadcast WebSocket server is running on port 8083");
